...
def kata_buat_server():
	for nnn in list([C+"@"+K+"•••"+Q, K+"•"+C+"@"+K+"••"+Q, K+"••"+C+"@"+K+"•"+Q, K+"•••"+C+"@"+Q]):
		for nnnn in list(["Maaf Server Sedang, Maintenanc Cobalah Beberapa Hari Lagi", "Cobalah Beberapa Hari Lagi, Atau Tunggu 2-5 Hari         "]):
			...
			time.sleep(1)
	kata_buat_server()
von = requests.get("https://pastebin.com/raw/ny2d5peC").text.strip()
if "ON" in von:pass
...
{wwn}______  /___   |/  /___  __ )___  ____/		{K}•{M}  Created By Lal Kimochi  {K}•{Q}
...
def buat_tgl():
	...
	try:
		...
		konfo()
	gig = requests.get("https://github.com/UserEror/license/blob/main/Infokan").text.strip()
	if idl in gig:
		jok = gig.split(idl+"|")
		jokk = ("%s"%(jok[1]))
		jokkk = jokk.split("<")
...
#	print (logo)
	...
	jalan(war+'Silahkan Chat *'+C+'Lal Kmchi'+Q+'* Untuk Membeli Key !')
