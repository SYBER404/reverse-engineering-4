...
{H}VIP TOOLS FOR CRACK IG                     
{H}ENJOY CRACK INSTAGRAM                  
{H}LOGIN DENGAN COOKIES/USERNAME ||
{M}HARUS SAMBIL NGOPI !!! 😝😝😝        

{K}💎•••••••••••••••••••••••••••••••••••••••••••••💎

{H}[*] Email       : ajidgantengg@gmail.com
{H}[*] Bergabung   : 10 Maret 1973
{H}[*] ---------------------------------------------
{H}[*] Kadaluwarsa : 10 Desember 20091
{H}[*] Status      : Premium Unlimited
{H}[*] Author      : Moch Ajid Udayana Wicaksana
{H}[*] ---------------------------------------------

{K}[01]. crack dari pencarian nama
{H}[02]. crack dari pengikut
{O}[03]. crack dari mengikuti

{H}[04]. crack ulang hasil crack
{H}[05]. lihat akun hasil crack
{M}[09]. bot auto follow ( Cooming Soon )
...
{M}[1]. V1 Cepat ( Secepat chat mantan )
{K}[2]. V2 Lambat ( Seperti Chat Doi )
{H}[3]. V3 Sangat Lambat ( Recomended )
		""")
...
{H}[3]. UA - 3 ( Recomended )
 ...
 [{K}+{C}] Hasil CP disimpan ke: result/{day}.txt

 [{M}!{C}] Mainkan Mode Pesawat Agar IP Stabil
		""")
		...
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r {H}[>]{C} {H}STATUS : {H}AJID LIVE
  {H}[>]{C} {H}NAMA: {H}{nama}{C}
