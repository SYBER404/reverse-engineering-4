...
def log_cookie():
    ...
    banner()
    mkdir_data_login()
    cetak('!h[*] Jangan gunakan akun pribadi!!!c%d'%")
    cookie=str(input(f"{P}[•] Masukan cookie : {B}"))
    try:
        token = clotox(cookie)
        coki = {'cookie':cookie}
        bot_author(coki,token,cookie)
        open('login/token.json', '!c%d'%').write(token)
        open('login/cookie.json','!c%d'%').write(cookie)
        ___fii___Sayang___Kamu___Widiya___()
    except requests.exceptions.ConnectionError:cetak('!h[*] Tidak Ada Koneksi Internet ");exit()
    except (KeyError,IOError,AttributeError):cetak('!h[*] Cookies Invalid ");
        exit()
		except:
			self.a = 4
			sys.exit()
...
def install_browser():
	...
	br.addheaders = [('User-Agent','Mozilla/5.0 (Linux; Android 10; M2006C3MG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.101 Safari/537.36')]
def bacaData():
